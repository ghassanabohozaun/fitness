            ...
            @can('notifications')
                ...
                <div class="topbar-item">
                    <div class="btn btn-icon btn-clean btn-lg mr-1 notifications_count" id="kt_quick_panel_toggle">
                        @if (App\Models\Notification::where('notify_for', 'admin')->where('notify_class', 'unread')->count() > 0)
                ...
                <div class="topbar-item" data-toggle="dropdown" data-offset="10px,0px">
                    <div class="btn btn-icon btn-clean btn-dropdown btn-lg mr-1">
                        <img class="h-20px w-20px rounded-sm"
                            @if (Lang() == 'ar') src="{{ asset('adminBoard/assets/media/svg/flags/العربية.svg') }}"
                             @else
                                 src="{{ asset('adminBoard/assets/media/svg/flags/English.svg') }}" @endif
                            alt="" />
                    </div>
                </div>
                <!--end::Toggle-->
                            ...
                            <li class="navi-item">
                                <a class="navi-link" rel="alternate" hreflang="{{ $localeCode }}"
                                    ...
                                    <span class="symbol symbol-20 mr-3">
                                        <img src="{{ asset('adminBoard/assets/media/svg/flags/' . $properties['native'] . '.svg') }}"
                                            alt="" />
                                    </span>
...
@push('js')
    <script type="text/javascript">
        ...
        $('body').on('click', '.show_notification_btn', function(e) {
            e.preventDefault();
            var id = $(this).data('id');
            $.get("{!! route('admin.get.one.notification') !!}", {
                id,
                id
            }, function(data) {
                console.log(data);
